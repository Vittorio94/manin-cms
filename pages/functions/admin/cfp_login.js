...
export async function onRequestPost(context) {
  ...
  const redirectPath = redirect.toString() || "/admin/";

  console.log(hashedPassword);
  console.log(hashedCfpPassword);

  if (hashedPassword === hashedCfpPassword) {
    // Valid password. Redirect to admin page and set cookie with auth hash.
    const cookieKeyValue = await getCookieKeyValue(env.CFP_PASSWORD);

    return new Response("", {
      status: 302,
      headers: {
        "Set-Cookie": `${cookieKeyValue}; Max-Age=${CFP_COOKIE_MAX_AGE}; Path=/; HttpOnly; Secure; SameSite=None`,
